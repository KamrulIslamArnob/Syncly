...
- **Read [DESIGN.md](../DESIGN.md)** for design system rules (typography, spacing, color, anti-patterns).
...
- **Storage** — use `chrome.storage.local` (never sync) for user data, except `aiQuotaPrefs` which uses sync.
- **Keep CSS tokens and JS theme in sync** — `tokens.css` and `penta-bridge/theme.js` must stay consistent.
- **All user data in `chrome.storage.local`** — never use `chrome.storage.sync` except for `aiQuotaPrefs`.

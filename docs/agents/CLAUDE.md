...
- **Sidebar & Header**: Quick nav: `Home` (house icon), `Quickie` (inbox icon, drop target), `Collections` (stacked-books icon, with the collections listed underneath while it is open). Folder tree section is titled `BOOKMARKS`; each folder's icon carries its colour (click the icon to pick another). The header shows the view's title and link count, a Compact/List/Grid switch, Add, the theme toggle and a ⋮ menu holding Select (multi-select checkboxes plus a floating bulk action bar for collections), Add Folder and Focus mode. Home's header shows the greeting and a To-Do button instead.
...
- **Freshness & folder views**: `shared/signal.js` bands each link live / resting / cold from `bookmarkUsage`, `bookmarkLastOpenedAt` (both written by `_recordOpen`) and Chrome's own `dateLastUsed`/`dateAdded`; opens counted before timestamps existed are dated to `bookmarkSignalSince`. The bands drive the card meta line (folder · last opened · open count, count shown in grid only), the signal strip with Prune above a folder's grid, and Home's triage row (Quickie to file, gone cold, duplicates), each opening its own list view (`cold`, `duplicates` selection types). A folder view lists every link nested inside it; its sub-folder chips narrow the grid in place.

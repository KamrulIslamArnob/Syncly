...
function inject() {
  ...
  const seed = {
    settings: { timeFormat: "24h", userName: "Probe", background: { kind: "solid_color", value: "#000000" }, shortcutsEnabled: true, greetingEnabled: true, clockEnabled: true },
  };
  window.chrome = {
...
try {
  ...
  const home = await page.evaluate(() => ({
    shell: !!document.querySelector(".bookmarks-shell"),
    fatal: document.querySelector(".fatal-error-details")?.textContent?.slice(0, 400) || "",
    stageLen: document.getElementById("stage")?.innerHTML.length || 0,
  }));
